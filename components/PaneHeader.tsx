      ...
      <div className="flex items-center gap-1">
        ...
        {showMinimizeButton && (
          <button
            ...
            title={t('paneheader.minimize')}
            aria-label={`${t('paneheader.minimize')} ${title}`}
          >
            <svg
        ...
        {showMaximizeButton && (
          <button
            onClick={onMaximize}
            className="p-1.5 hover:bg-slate-200 rounded transition-colors"
            title={t('paneheader.maximize')}
            aria-label={`${t('paneheader.maximize')} ${title}`}
          >
            <svg
              className="w-4 h-4 text-slate-600"
              fill="none"
        ...
        {showRestoreButton && (
          <button
            onClick={onRestore}
            className="p-1.5 hover:bg-slate-200 rounded transition-colors"
            title={t('paneheader.restore')}
            aria-label={`${t('paneheader.restore')} ${title}`}
          >
            <svg
              className="w-4 h-4 text-slate-600"
              fill="none"
