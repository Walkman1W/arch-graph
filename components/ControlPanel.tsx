...
const ControlPanel: React.FC<ControlPanelProps> = ({ onCommandProcessed, filteredCount }) => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      text: t('control.greeting'),
      timestamp: new Date(),
      suggestions: [
        { label: language === 'zh' ? '隔离结构' : 'Isolate Structure', payload: { operation: 'ISOLATE' as any, category: 'Columns', level: null, material: null } },
        { label: language === 'zh' ? '显示一层' : 'Show Level 1', payload: { operation: 'ISOLATE' as any, category: null, level: 'Level 1', material: null } },
        { label: language === 'zh' ? '重置视图' : 'Reset View', payload: { operation: 'RESET' as any, category: null, level: null, material: null } }
      ]
    }
  ]);
  ...
  const handleSendMessage = async (textOverride?: string) => {
    ...
    try {
      ...
      const errorMsg: Message = {
        ...
        role: 'assistant',
        text: language === 'zh' 
          ? "连接 AI 服务时遇到错误。请检查你的 API 密钥。"
          : "I encountered an error connecting to the AI service. Please check your API key.",
        timestamp: new Date()
      };
  ...
  const handleSuggestionClick = (suggestion: BIMSuggestion) => {
    ...
    const userMsg: Message = {
      id: Date.now().toString(),
      role: 'user',
      text: `${t('control.selected')}: ${suggestion.label}`,
      timestamp: new Date()
    };
    setMessages(prev => [...prev, userMsg]);
    ...
    const fullResponse: BIMQueryResponse = {
      ...
      reasoning: language === 'zh' 
        ? `已应用操作"${suggestion.label}"。`
        : `Action "${suggestion.label}" applied.`,
      suggestions: []
    };
    ...
    setTimeout(() => {
      setMessages(prev => [...prev, {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        text: `${t('control.applied')}: ${suggestion.label}`,
        timestamp: new Date()
      }]);
    }, 400);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-white border-l border-slate-200 shadow-xl w-full md:w-[400px]">
      ...
      <div className="px-4 py-2 bg-slate-50 border-t border-slate-100 flex gap-2 overflow-x-auto no-scrollbar items-center">
         <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider whitespace-nowrap mr-1">{t('control.aiTools')}</span>
         <button 
            onClick={() => handleSendMessage(t('ai.analyzeStructural'))}
         ...
         >
            <span className="text-lg">🏗️</span> {t('control.analyzeStructure')}
         </button>
         <button 
            onClick={() => handleSendMessage(t('ai.showMechanical'))}
         ...
         >
            <span className="text-lg">🔧</span> {t('control.mepCheck')}
         </button>
      </div>
      ...
      <div className="p-4 bg-white border-t border-slate-200 pt-2">
        <div className="relative flex items-center">
          <input
            ...
            placeholder={t('control.inputPlaceholder')}
          ...
          <div className="absolute right-2 flex items-center gap-1">
            <button
              ...
              title={t('control.microphoneTitle')}
            >
