...
const ControlPanel: React.FC<ControlPanelProps> = ({ onCommandProcessed, filteredCount }) => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      text: t('controlPanel.initialMessage'),
      timestamp: new Date(),
      suggestions: [
        { label: t('controlPanel.suggestion.isolateStructure'), payload: { operation: 'ISOLATE' as any, category: 'Columns', level: null, material: null } },
        { label: t('controlPanel.suggestion.showLevel1'), payload: { operation: 'ISOLATE' as any, category: null, level: 'Level 1', material: null } },
        { label: t('controlPanel.suggestion.resetView'), payload: { operation: 'RESET' as any, category: null, level: null, material: null } }
      ]
    }
  ]);
  ...
  const handleSuggestionClick = (suggestion: BIMSuggestion) => {
    ...
    const userMsg: Message = {
      id: Date.now().toString(),
      role: 'user',
      text: `${t('message.selected')}${suggestion.label}`,
      timestamp: new Date()
    };
    setMessages(prev => [...prev, userMsg]);
    ...
    const fullResponse: BIMQueryResponse = {
      ...suggestion.payload,
      keywords: [],
      reasoning: `${t('message.actionApplied')}${suggestion.label}`,
      suggestions: []
    };
    ...
    setTimeout(() => {
      setMessages(prev => [...prev, {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        text: `${t('controlPanel.appliedFilter')}${suggestion.label}`,
        timestamp: new Date()
      }]);
    }, 400);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-white border-l border-slate-200 shadow-xl w-full md:w-[400px]">
      ...
      <div className="px-4 py-2 bg-slate-50 border-t border-slate-100 flex gap-2 overflow-x-auto no-scrollbar items-center">
         ...
         >
            <span className="text-lg">🏗️</span> {t('controlPanel.analyzeStructure')}
         </button>
         <button 
         ...
         >
            <span className="text-lg">🔧</span> {t('controlPanel.mepCheck')}
         </button>
      </div>
